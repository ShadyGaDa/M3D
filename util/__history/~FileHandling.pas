...
{$REGION '============================================================================= > | VARIABLES'}
// backup old ini
var IniVal: array[0..36] of String = (
      '81',      {StickOption       }
      '5',       {MandRotDeg        }
      '20',      {NavSlideStep      }
      '5',       {NavLookAngle      }
...
const
    IniMax: Integer = 36;
    IniItem: array[0..36] of String = (
      'StickOption',
      'MandRotDeg',
      'NavSlideStep',
      'NavLookAngle',
      'NavFarPlane',
      'NavFOVy',          //NavFarPlane not used anymore
      'NavMaxIts',
      'AniFrameCount',
      'AniPrevFPS',
      'AniSmoothPar',
      ...
      'VisualTheme',
      'NaviSize');
    AccPresetFileNames: array[0..3] of String = (
      'preset_preview.txt',
      'preset_video.txt',
      'preset_mid.txt',
      'preset_high.txt');
    AccPresetItemNames: array[0..7] of String = (
      'SmoothNormals',
      'DEstop',
      'DEaccuracy',
      'BinSearch',
...
uses
  ...
  uTEST;
//FormulaGUI
//Animation, AniPreviewWindow, Tiling
...
function LoadColPreset(nr: Integer): Boolean;  //6-15  saved as 1-10
var s: String;
    f: File;
    i: Integer;
    tP: TLpreset16;
begin
    Result := False;
    s := AppDataDir + 'color_preset_' + IntToStr(nr-5) + '.bin';
    ...
    begin
      ...
      try
        ...
        begin
          ...
          CustomPresets[nr] := ConvertColPreset16To20(@tP);
        end
        else BlockRead(f, CustomPresets[nr], SizeOf(TLpreset20));
        UpdatePresetFrom20(CustomPresets[nr]);
        Result := True;
      finally
        CloseFile(f);
      end;
...
procedure SaveColPreset(nr: Integer); //6..15 = CustomPresets[6..15]
var s: String;
    f: File;
begin
    s := AppDataDir + 'color_preset_' + IntToStr(nr-5) + '.bin';
    ...
    try
      Rewrite(f, 1);
      CustomPresets[nr].Version := actPresetVersion;
      BlockWrite(f, CustomPresets[nr], SizeOf(TLpreset20));
    finally
      CloseFile(f);
    end;
end;
...
begin
    ...
    begin
      ...
      ChangeFName(SaveDialog_L_1, s);{LIGHT}
      Caption := s;
    end;
end;

procedure StoreHistoryPars(var pars: TMandHeader10);
var tmpHF,s,si: String;
    dt: TDateTime;
    f: file;
    i: Integer;
begin //todo: store last header+haddon, comparemem of both if changed then save ... +verify every 5 minutes (lighting+postpro changes)
  try
    ...
    if not GUI.bRandom then
    begin
    //    s:=FormatDateTime('zzz');
    //s := Format('%.*d,[length, number]);
    i := 1 + Random(999999999);
    //ShowMessage(Format('<%.9d>', IntToStr(i)));
    s:=Format('%.9d', [i] );
    //si:=RightStr(StringOfChar('0',9) + IntToStr(i), 9 );
    //s := Format('%.*d,[9, si]);

    if GUI.Caption = 'Mandelbulb 3D v1.9' then GUI.Caption := s;
    if GUI.Caption = 'Default' then GUI.Caption := s;
    if GUI.Caption = 'Default~' then GUI.Caption := s;
    if GUI.Caption = 'main paras~' then GUI.Caption := s;
    //if GUI.Caption <> 'Mandelbulb 3D' then tmpHF := tmpHF + '{' + ChangeFileExt(GUI.Caption, '}');
    end;
    tmpHF := tmpHF + ChangeFileExt('{' + GUI.Caption + '}', '.m3p' );
    try
      AssignFile(f, tmpHF);
      Rewrite(f, 1);
  ...
  try
   ...
   { NormalZ:    SmallInt;  negate
    ...
    Zpos:       Word;   }
      ...
      MakeHistory;{LIGHT}
      if bTrigRepaint then GUI.TriggerRepaint;
    end;
  finally
    Screen.Cursor := crDefault;
  end;
    ...
    //    sDEcolVar := 0.521;
      ...
      begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                Result := (not LoadFs) or LoadCustomFormulaFromHeaderFull(PTHeaderCustomAddon(PCFAddon).Formulas[i].CustomFname,
        ...
        begin
          ...
          Result := (not LoadFs) or LoadCustomFormulaFromHeaderFull(PTHeaderCustomAddon(PCFAddon).Formulas[0].CustomFname,
        ...
        0, 10:  begin
         ...
         5, 8:  begin   //Box
          ...
          begin
            ...
            Result := (not LoadFs) or LoadCustomFormulaFromHeaderFull(CustomFname, PTCustomFormula(PHCustomF[0])^,
            ...
            begin
              ...
              begin  //load also additional  _SinePow2, IntP2, MaxClip
                ...
                if LoadFs then LoadCustomFormulaFromHeaderFull(CustomFname,
...
//procedure StoreHistoryParsMutagen(var HEADER: TMandHeader10);
...
begin//todo: De. Duplication
      try
        ...
        try
          ...
          AssignFile(f, tmpHF);
          Rewrite(f, 1);
          InsertAuthorsToPara(@HEADER, GUI.Authors);  //change author c.o. MutaGen
          try
          ...
          PTHeaderCustomAddon(HEADER.PCFAddon)^.bHCAversion := 16;
          BlockWrite(f, PTHeaderCustomAddon(HEADER.PCFAddon)^, SizeOf(THeaderCustomAddon));
        finally
          CloseFile(f);
        end;
        LastHisParSaveTime := Now;
