...
procedure CheckDEoption(const startNr, endNr: Integer;
                        pHCA: PTHeaderCustomAddon;
                        const PHCustomF: array of Pointer;
                        var DEoption: Integer);
...
begin
    ...
    begin
      ...
      if (de = -1) or (de = 21) or (DEoption = de) then Continue;
      if de = 20 then Inc(difsShapes);
      if ((DEoption in [0..11]) and (de > 11)) or ((DEoption > 11) and (de in [0..11])) then  //11/19
      begin
        DEoption := -1;
        Break;
      end
...
procedure CheckFormulaOptions(const startNr, endNr: Integer;
                              pHCA: PTHeaderCustomAddon;
                              const PHCustomF: array of Pointer;
                              var DEoption: Integer;
                              var MandFunctionDE: TMandFunctionDE;
                              var MandFunction: TMandFunction;
                              var nHybrid: array of Integer;
                              const bDisableADE: LongBool);
...
begin
    ...
    begin
      LastF := i;
      de := PTCustomFormula(PHCustomF[i]).iDEoption;
      if (de < 0) or (de > 20) then nHybrid[i] := -Abs(nHybrid[i]) else LastValidF := i;
    ...
    begin
      ...
      if (pHCA.bOptions1 = 2) and (pHCA.bOptions3 = 5) and ((DEoption = -1 ) or (DEoption = 21)) then //< 0  > 20
      for i := startNr to endNr do if nHybrid[i] <> 0 then
      begin
...
begin
    ...
    begin
      ...
      pBGRotMatrix := @GUINav.BGpicRotMatrix;
      if iBackBMP <> 0 then
      with M3DBackGroundPic do
      begin
        if LMWidth > 0 then
