...
begin
    ...
    else  //changed to msecs
     Result := FormatDateTime('h"h"nn"m"ss"s"', ms); // 'h:nn:ss'
//      Result := FormatDateTime('h"h"nn"m"ss"s"z"zzz"', ms); // 'h:nn:ss'
end;
...
var d: Double;                                                      //lightpos, rel. to mid vals!
    i: Integer;
    M: TMatrix3;
    V: TVec3D;
begin
...
function IntToTimeStr(i: Integer): String;
begin
    if i >= 36000 then Result := FormatDateTime('h:nn:ss.zzz', i / 864000)
    else
    if i > 0 then Result := FormatDateTime('n:ss.zzz', i / 864000) + '.' + IntToStr(i mod 10)
    else
      Result := '-';
end;
