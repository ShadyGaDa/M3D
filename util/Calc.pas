unit Calc;
//nb. many unused methods removed
interface
...
function CalcMandT(Header: TPMandHeader10;
                   PLightVals: TPLightVals;
                   PCTS: TPCalcThreadStats;
                   PsiLight5: TPsiLight5;
                   hSLoffset, FSIstart, FSIoffset: Integer;
                   hRect: TRect): Boolean;
...
function CalcDEfull(It3Dex: TPIteration3Dext; mctp: Pointer): Double; {PMCTparameter}
function CalcDEanalytic(It3Dex: TPIteration3Dext; mctp: Pointer): Double;{PMCTparameter}
...
function GetMapPixelDirectXYspline(PVec3D: TPVec3D; MapNr: Integer): TVec3D;
procedure DelayCalcPart(ThreadCount: Integer; PCTS: TPCalcThreadStats);
procedure IniCalcMaps( Header: TPMandHeader10); {MCTparas: PMCTparameter;}
function SplineIpolMap(x, y: Double; PCalcMap: TPLightMap): TVec3D;
procedure RMdoBinSearch(pMCTparas: PMCTparameter; var DE, RLastStepWidth: Double); {, LastDE}
...
var CalcMaps: array[0..2] of TLightMap; //max 3 lightmaps?   todo: Global Map Handler with depencies + exclusive free+load
    MapIsMissing: LongBool; //  CalcDE: TCaldDEfunction = CalcDEfull;
...
begin
  ...
  try
    ...
    if Result then begin
      ...
      if MCTparas.calc3D then SetLength(MandCalcThread, ThreadCount) else SetLength(MandCalcThread2D, ThreadCount);
    end;
...
begin
    ...
    begin
      ...
      if bCalcInside then
      begin
        //DEoptionResult.. in DEcomb the one who rules the raystep (for faster insides in decomb)
        if DEoptionResult = 20 then Result := msDEstop * 2 - Result
        else
        begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          mMandFunction(@It3Dex.C1);
          Rst       := Sqr(bufRout - It3Dex.Rout); //overflow
          It3Dex.C3 := bufD;
          ...
          It3Dex.MaxIt := bufMaxIt; //  It3Dex.SmoothItD := bufIt3DSmoothItD;
          PInteger(@It3Dex.CalcSIT)^ := bufMCTCalcSIT;
          It3Dex.RStop := dRStop; //      mCopyVec(@It3Dex.C1, @DEcombRec.bufVec3D);
        end;
      end;
      MaxItsResult := It3Dex.MaxIt;
      if Result < msDEstop * 0.25 then Result := msDEstop * 0.25;  //MaxCDvar(DS025, Rst);  ! NAN !
      DEoptionResult := It3Dex.DEoption;
      if bCalcInside then
      begin
        Result := msDEstop * 4 - Result * 3;
    ...
    begin                     //3d gradient
      ...
      begin
        ...
        begin
          ...
          Result := DEcombRec.bufIt3DOtrap * Ln(DEcombRec.bufIt3DOtrap) * dDEscale / (Sqrt(Rst + wt + dt) + mctDEoffset006);
...
procedure RMCalculateNormals(pMCTparas: PMCTparameter; var NN: Single);
var itmp, iy, ix, SmoothN: Integer; //   bIR: LongBool;
...
@@6:
    ...
    pop ebx   //}
                                   ...
                                   //eax             edx      ecx          ebp+8
procedure RMdoBinSearch(pMCTparas: PMCTparameter; var DE, RLastStepWidth: Double); {, RLastDE}
...
//procedure RayMarchVV(RMrec: TPRaymarchRec);
...
//procedure CopyFormulas(const OldFormulas: array of TCustomFormula; MCTparas: PMCTparameter; var CFormulas: array of TFormulaClass);

{
procedure CopyFormulas(const OldFormulas: array of TCustomFormula;
                      MCTparas: PMCTparameter;
                      var CFormulas: array of TFormulaClass);
var i: Integer;
begin
    for i := 0 to 5 do
    begin
 ...
 //     if not IsCustomDE then Inc(pIt3Dext.maxIt, 4);   //test
   ...
   //   if not IsCustomDE then Dec(pIt3Dext.maxIt, 4);  //test
     ...
     //  bInsideRendering := bIR;  }
      ...
      {   from CalcDEnoADE       //   bufIt3DSmoothItD: Single;
