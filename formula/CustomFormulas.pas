...
procedure ParseCFfromOld(iFormula: Integer; CF: PTCustomFormula; dOptionValues: array of Double);
...
procedure CopyTypeAndOptionFromCFtoHAddon(CF: PTCustomFormula; HA: PTHeaderCustomAddon; fnr: Integer);
var i: Integer;
begin
    for i := 0 to 15 do HA.Formulas[fnr].byOptionType[i] := CF.byOptionTypes[i];
...
begin
    ...
    begin
      ...
      if bCPmemReserved and (Pointer(pCodePointer) <> nil) then VirtualFree(pCodePointer, {0}4096, {MEM_RELEASE} MEM_DECOMMIT);
...
const MemNeeded: array[0..22] of Integer = (8,4,4,16,8,72,36,16,40,8,0,32,64,8,16,8,8,16,36,8,8,8,16);
begin
    ...
    begin    //iOptionTypes[i]     0      1       2         3          4            5              6         7 (MinR) 8  Rthr   9 Double, will be squared
      ...
      begin
        ...
        case byOptionTypes[i] of
          ...
          5: begin
               BuildRotMatrix(dOptionValues[i] * Pid180, dOptionValues[i + 1] * Pid180, dOptionValues[i + 2] * Pid180, @M);
               for j := 0 to 8 do
               begin
                 Dec(p, 2);
                 PDouble(p)^ := pd^;
      ...
      16,17: begin
         ...
         18: begin  //3SCALESANGLES
               BuildRotMatrix(dOptionValues[i + 1] * Pid180, dOptionValues[i + 2] * Pid180, dOptionValues[i + 3] * Pid180, @M);
...
begin
    ...
    begin
      ...
      if (not bCPmemReserved) and CF2.bCPmemReserved then pCodePointer := VirtualAlloc(nil, 4096, {MEM_RESERVE} MEM_COMMIT, PAGE_EXECUTE_READWRITE);
      if bCPmemReserved and (not CF2.bCPmemReserved) then VirtualFree(pCodePointer, {0}4096, {MEM_RELEASE} MEM_DECOMMIT);
...
var n, iVersion, SIMDlevel: Integer;
    ...
    s: String;
const
    //cOptions: String = '.SSE2.SSE3.DEscale.SIpower.RStop.Version.DEoption';
    //cOptions: String = '.SSE2.DESC.SIPO.RSTO.SSE3.SSSE.SSE4.VERS.DEOPTION';
    //cOptions: String = '.SSE2.SSE3.VERS.DEOP.DESC.SIPO.RSTO';
    cOptions: String = '.SSE2.SSE3.VERS.DEOP';
      ...
      begin
        repeat
          ...
          case n of
            ...
            6:   SIMDlevel  := SIMDlevel or 2;            //21
            11:  iVersion   := StrToInt(StrLastWord(s));  //36
            16:  DEfunction := StrToInt(StrLastWord(s));  //41
            //26:  SIMDlevel := SIMDlevel or 4;     //not used
            //31:  SIMDlevel := SIMDlevel or 8;     //not used
          end;
        until EOF(f) or (s[1] = '[');
      end;
...
function CanLoadF(FormulaName: String): Boolean;
var df: Integer;    // just check DEOption  SIMDlevel(.SSE2.SSE3)  Version[2..9]
begin
...
//    cOptions: String = '.SSE2.DESC.SIPO.RSTO.SSE3.SSSE.SSE4.VERS.DEOP.DIFS.ADES.';
    cOptions: String = '.SSE2.SSE3.DESC.SIPO.RSTO.VERS.DEOP.ADES.'; //  .SSSE.SSE4.DIFS
...
//------------------------------------------------------------------------------
                      ...
                      // 1      8      15      23          35          47            61            75       84      92      100        111       121           129      138      148
                      //
...
begin
    ...
    Result := LoadCustomFormula(IncludeTrailingPathDelimiter(IniDirs[3]) + s, lCustomFormula, CustomFname, dOptionVars, False, 0, nil, False);
    if not Result then
    begin
...
begin
    ...
    begin
      CheckDEoption(PCFA.bHybOpt2 and 7, (PCFA.bHybOpt2 shr 4) and 7, PCFA, Header.PHCustomF, DEopt);
...
begin
 ...
 {    //CopyTypeAndOptionFromCFtoHAddon        ; bLoadedPars: LongBool
    if bLoadedPars and (HA.Formulas[fnr].iOptionCount < CF.iCFOptionCount) then
    begin  // HA.Formulas[fnr].wOptionCountLoaded := HA.Formulas[fnr].wOptionCount;
           // + fill all above loadedcount with defaults when loading formula?
